...
function promptUserAuth(code) {
  log.alert('This script needs to do a one-time authorization in order ' +
      ...
      'In your browser, go to %s and enter the following code: %s',
      'http://google.com/device'.red, code.red);
}

function onAccessDenied() {
  log.error('Error: '.red + 'You have denied the request to access your ' +
