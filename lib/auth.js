...
function handleNewTokenData(data) {
  ...
  // Return whether or not new data was received.
  return !!data;
}

function saveTokensToDisk(file, data) {
  log.trace(messages.auth.SAVING_TOKENS, file);
  return fsp.outputJson(file, data);
}
...
module.exports = {
  ...
  getAccessToken: function() {
    ...
    return fsp.readJson(tokenFile)
        ...
        .then(function(needsSaving) {
          if (needsSaving) {
            log.trace(messages.auth.SAVING_TOKENS, tokenFile);
            return saveTokensToDisk(tokenFile, this);
          }
        })
        .then(function() {
