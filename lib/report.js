...
function parseVersion(version, browser) {
  var pattern = /^v?(\d+)\.?(\d+)?/i;
  var matches = pattern.exec(version);
  if (!matches) {
    log('Note: could not parse ' + browser + ' version "' + version + '".');
    return '(unknown)';
  }
  ...
  if (browser == 'Safari' && majorVersion > 10) {
    log('Note: ignoring Safari result reporting Webkit version ' +
        version + '.');
    return '(unknown)';
  }
...
module.exports = {
  ...
  output: function(results) {
    ...
    return writeJSON(this.config.outputFile, output);
  }
}